...
func UploadFile(c *gin.Context, s3Region string, s3Bucket string, s3Root string) {
	gcpPrefix := "gcp:" // TODO refactor this hack
	if strings.HasPrefix(s3Region, gcpPrefix) {
		s3Region = s3Region[len(gcpPrefix):]
		uploadFileGcs(c, s3Region, s3Bucket, s3Root)
	} else {
		uploadFileS3(c, s3Region, s3Bucket, s3Root)
	}
}

func uploadFileS3(c *gin.Context, s3Region string, s3Bucket string, s3Root string) {
